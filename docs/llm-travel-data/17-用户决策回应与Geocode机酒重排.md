...
| 层级 | 一句话 | 用户看到什么算「准」 | 本项目现状 |
|------|--------|----------------------|------------|
| **L1 地图可落地** | 点在对的城市、大致对的片区 | 地图上「滨海湾金沙」在新加坡滨海湾，而不是纽约或海里 | **坏了**：生成后 geocode 常错点 |
| **L2 机酒锚点** | 行程服从「何时到/何时走、住哪一带」 | Day1 不早于抵达；住宿在推荐片区附近 | **半成品**：Panel 能确认，但 generate **不吃** `travel_intel`（B-P4） |
...
| 序 | 项 | 状态 |
|----|-----|------|
| 1 | **GEO 修复**：目的地围栏 + Top1 校验 + 可疑坐标警告 | P0 · 对准主诉 |
| 2 | 机票 rank → **5:2:2:1** + 到达维 | P0 · 对齐 [18](./18-机酒优先与迭代行程产品决策.md) |
| 3 | **B-P4** + 门禁先机酒再精排；迭代 optimize/regenerate | P0 |
| 4 | 深链 UX 审计（禁止自动 open）+ HOT-03 每晚预算 | P1 |
| 5 | **HOT-01** 第二酒店源探路（多方比价后置） | P1 |
| 6 | 天气 / Tavily / 完整 Planner | ⏸ |
