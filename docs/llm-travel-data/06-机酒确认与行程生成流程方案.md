...
7. **品类停留 L1（P95 / B-P4-06）**：`visit_duration` 品类 `(min,typical,max)` 与 prompt 同表；generate 后夹逼离谱区间再接龙；**不**用 Tavily/联网搜营业时间；玩法印证仍只服务「去哪」
...
| 预订 | Trip.com Deep Link 写入整单 `purchase.url`（主 CTA）；Ignav `booking-links` 写入该报价 `purchase_url` |
| UI（产品决策 2026-08-11） | **有 Ignav 购买链则展示**（报价卡 / 确认态），与 Trip CTA 并存；无链则只保留 Trip。标签勿一律写「Trip.com」（可能是航司直链）。跟踪 **B-FLT-09** / [P124](../问题日志.md) |
